...
from fixingdata import PointCloudLowPassFilter
...
def update():
    ...
    if dataOk:
        if detObj.get("numObj", 0) > 0:
            ...
            filtered_x, filtered_y = visualizationFilter.apply(raw_x, raw_y)
            s.setData(filtered_x, filtered_y)
        else:
            ...
            filtered_x, filtered_y = visualizationFilter.apply([], [])
            s.setData(filtered_x, filtered_y)
